...
public class OptimalLineup {
		...
		for (int j = i + 1; j < defenders.size(); j++) {
			for (int k = j + 1; k < defenders.size(); k++) {
				for (int l = k + 1; l < defenders.size(); l++) {
					for (int m = l + 1; m < defenders.size(); m++) {
						Lineup lineup = new Lineup(budget, 3);
				        lineup.add(defenders.get(i));
				        lineup.add(defenders.get(j));
				        lineup.add(defenders.get(k));
				        if (lineup.canAdd(defenders.get(l))) {
				        	lineup.add(defenders.get(l));
				        	if (lineup.canAdd(defenders.get(m))) {
		    			        lineup.add(defenders.get(m));
		    			        float points = lineup.getExpectedPoints();
		    			        int price = lineup.getTotalPrice();
  ...
  private int tryAttackingLineups(Lineup lineup, int start) {
    // TODO: Implement branch and bound search to replace part from here >>>>>
	  ...
	  int sum = 0;
	  if (lineup.size() == 8) {						// already have all 8 attackers
	    float maxDefPoints = bestPointsDefense[lineup.getRemainingBudget()];
	    float attackPoints = lineup.getExpectedPoints();
	    if (maxDefPoints + attackPoints > bestPoints) {
    	  bestPoints = attackPoints + maxDefPoints;
    	  Lineup copy = lineup.clone();
    	  for (Player defender : bestPlayersDefense[lineup.getRemainingBudget()]) {
    		if (copy.canAdd(defender)) {  
    		  copy.add(defender);
    		}
    	  }
	      bestLineup = copy;
		...
		return 1;	
	  }		// lineup isn't complete, so we need to add attackers
    for (int i = start; i < attackers.size();) {
	  if (lineup.canAdd(attackers.get(i))) {
	    lineup.add(attackers.get(i));
	    int mid = 5 - lineup.sizeByPosition(Position.MID);
	    int fwd = 3 - lineup.sizeByPosition(Position.FWD);
	    LineupBound remainder = new LineupBound(lineup, 0, 0, 0, mid, fwd);
	    ListIterator<Player> subList = attackers.listIterator(start);
	    float upperBound = remainder.maxPoints(subList);
	    float completeBound = upperBound + bestPointsDefense[lineup.getRemainingBudget()];
	    if (completeBound - bestPoints > 1e-5) {
	      sum = sum + tryAttackingLineups(lineup, i + 1);		// explore, new recursive call
	    } else {
	      break;
	    }
	 	lineup.remove(attackers.get(i));							// remove player
	 	return sum + tryAttackingLineups(lineup, i + 1);			// try without this player and see if it's actually better
	  } else {														// can't add that player
 	    return sum + tryAttackingLineups(lineup, i + 1);
      }	  
	}
    return sum;
  }	  
